...
Sections tackled: 
1) Working with Character Data - Completed the Exercises from Strings chapter in R for Data Science    
...
Here, we re-set the parameters for the plot so that we make the intercept match up to 1952, our earliest datapoint:
```{r}
...
Would it also be more useful to have a way to directly plot either a linear or quadratic fit into ggplot data?
```{r}
...
quadraticfit_ggplot("Germany")
linearfit_ggplot("Germany")
```
...
quadratic_regression_fit("Canada")
quadratic_regression_fit("Kenya")
quadratic_regression_fit("Spain")

quadraticfit_ggplot("Spain")
```

#use coefficients to make a Life expectancy estimator for a certain year.
